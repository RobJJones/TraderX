...
public class BaseController2 implements EWrapper{
    ...
    @Override public void nextValidId(int orderId) {
        ...
        m_reqId = Util.getCurrentOrderId() + 1000000000; // let order id's not collide with other request id's
        m_connected  = true;
    ...
    public void updateGroups( List<Group> groups) {
        ...
        m_client.replaceFA( REPLACE_FA_GROUPS_REQ_ID, Types.FADataType.GROUPS.ordinal(), AdvisorUtil.getGroupsXml( groups) );
        sendEOM();
    }
    ...
    public void updateProfiles(List<Profile> profiles) {
        ...
        m_client.replaceFA( REPLACE_FA_PROFILES_REQ_ID, Types.FADataType.PROFILES.ordinal(), AdvisorUtil.getProfilesXml( profiles) );
        sendEOM();
    }
    ...
    public void placeOrModifyOrder(Contract contract, final List<Order> bracketOrder) {
        if (!checkConnection())
            return;

        for(Order order : bracketOrder) {

            if (!m_orderHandlers.containsKey(order.orderId())) {
                m_orderHandlers.put(order.orderId(), new OrderHandler());
            }
    ...
    public void placeOrModifyOrder(Contract contract, Order order, final IOrderHandler handler) {
        if (!checkConnection())
            return;
